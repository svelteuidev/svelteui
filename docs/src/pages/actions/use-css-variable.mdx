...
<p>
	With the <strong>{meta.title}</strong> action, an object of properties will be treated as css
	custom variables. By defining this object inside of a `$: {}` reactive block, {meta.title} can
	update those css properties on the fly whenever some of its values change.
</p>
