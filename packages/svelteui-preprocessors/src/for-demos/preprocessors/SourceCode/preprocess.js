...
function getSourceIdAttribute(node) {
    return node?.attributes?.find(({ name }) => name === 'sourceId')?.value?.[0]?.data;
}
function defaultSourceCodeMap() {
    return {
        foundSource: false,
        foundDisplay: false,
        displayNodesStart: [],
...
function cleanContent(content) {
    ...
    const maxLength = lines.reduce((prev, curr) => (prev.length > curr.length ? prev : curr))?.length ?? 0;
...
export default function preprocessSvelteViewSource(opts) {
    ...
    return {
        markup({ content, filename }) {
            ...
            const ast = parse(trimmedContent);
            const pairs = {};
            walk(ast.html, {
                enter(node) {
                    if (node.type === 'InlineComponent' &&
                        ...
                        if (node.name === SOURCE_NODE_NAME) {
                            if (pairs[sourceId].foundSource) {
                                throw Error(`Found a duplicate sourceId on a ${SOURCE_NODE_NAME}: ${sourceId}. sourceIds must be unique across ${SOURCE_NODE_NAME} elements in a Svelte file`);
                            }
                            if (node.children && node.children.length > 0) {
                                const contentStart = node.children[0].start;
                                const contentEnd = node.children[node.children.length - 1].end;
                                pairs[sourceId].content = jsStringEscape(cleanContent(s.slice(contentStart, contentEnd)));
                            }
                            else {
            ...
            Object.entries(pairs).forEach(([id, pair]) => {
                if (!(pair.foundSource && pair.foundDisplay)) {
                    throw Error(`Did not find a matching ${pair.foundSource ? DISPLAY_NODE_NAME : SOURCE_NODE_NAME} node for sourceId ${id}`);
                }
