...
describe('@mantine/styles/getSize', () => {
	...
	it('does nothing to size defined in other units', () => {
		expect(getSize({ size: '2rem' as any, sizes: { sm: 10, md: '5rem' } })).toBe('2rem');
		expect(getSize({ size: '10%' as any, sizes: { sm: 10, md: '5rem' } })).toBe('10%');
		expect(getSize({ size: '100vh' as any, sizes: { sm: 10, md: '5rem' } })).toBe('100vh');
	});
});
